...
#define UTF_8 "UTF-8"
#define LOG_TAG "encrypt_jni" // 自定义的LOG的标识
...
jboolean IS_DEBUG = true; //日志开关，true为开，其它为关
...
Java_com_rratchet_android_encrypt_EncryptHelper_setDebug(JNIEnv *env, jobject instance, jboolean isDebug) {
...
Java_com_rratchet_android_encrypt_EncryptHelper_createSecretKey(JNIEnv *env, jobject instance, jobject context) {

    jstring deviceId = getDeviceID(env, instance, context);
    jstring serialNumber = getSerialNumber(env, instance, context);

    jstring imei = getAppendedString(env, instance, deviceId, serialNumber);

    if (IS_DEBUG){
        LOGD("imei = %s", jstringTostring(env,imei));
    }
    jstring sign = getPublicKey(env, instance, context);
    if (IS_DEBUG){
        LOGD("sign = %s",jstringTostring(env,sign));
    }
    jstring imei_sign = getAppendedString(env, instance, imei, sign);
    if (IS_DEBUG){
        LOGD("imei_sign = %s",jstringTostring(env,imei_sign));
    }
    jstring package = getPackageName(env, instance, context);
    if (IS_DEBUG){
        LOGD("package = %s",jstringTostring(env,package));
    }
    jstring imei_sign_package = getAppendedString(env, instance, imei_sign,package);
    if (IS_DEBUG){
        LOGD("imei_sign_package = %s",jstringTostring(env,imei_sign_package));
    }
    //请再加入自己的移位或替换 或其他加密算法,例如我又append了一次imei
    imei_sign_package = getAppendedString(env, instance, imei_sign_package, imei);
    if (IS_DEBUG){
        LOGD("imei_sign_package2 = %s",jstringTostring(env,imei_sign_package));
    }

    imei_sign_package = getAppendedString(env, instance, imei_sign_package, sign);
    if (IS_DEBUG){
